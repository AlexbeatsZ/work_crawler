...
- 《屈曲ラヴァー》原始 EPUB 的 137 个 XHTML 均保留日文引号，`「`/`」` 分别为 9689/9691 个；Wenyi ingest 后 source 数量一致。后续中文引号遗漏发生在翻译/润色 target，不是 Kakuyomu 爬虫或 EPUB 打包造成。
...
- [x] 交叉核验原始 EPUB 与 Wenyi state，排除爬虫导致日文对话引号丢失。
